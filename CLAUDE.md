...
- [ ] Store credentials in keychain (one-time): `make notarize-store-creds APPLE_TEAM_ID=... APPLE_ID=... NOTARIZE_PROFILE=gridly`
- [ ] Ship: `make dist APPLE_TEAM_ID=... NOTARIZE_PROFILE=gridly` → produces `build/Gridly-1.0.dmg`
- [ ] Pick a storefront — Gumroad, your own site, or [Setapp](https://setapp.com) (no sandbox requirement)

## Build & Run

```sh
make app       # compile (release) + assemble build/Gridly.app + ad-hoc sign
...
If Xcode is installed, use `make xcode-build` or open `Gridly.xcodeproj` directly.
...
Swift Package Manager executable (`Sources/Gridly/`) assembled into a macOS menu-bar app.
The Xcode project (`Gridly.xcodeproj`) is generated by `xcodegen` from `project.yml`
...
**App Sandbox must remain disabled** (`Gridly.entitlements`) — the Accessibility API is incompatible with sandboxing. This means the **Mac App Store is not an option**. Distribute via direct download (Gumroad, your site, Setapp, etc.) with a notarized `.dmg`.
...
make dist APPLE_TEAM_ID=XXXXXXXXXX NOTARIZE_PROFILE=gridly
...
# Output: build/Gridly-1.0.dmg  (notarized, stapled, ready to ship)
```
...
make notarize   NOTARIZE_PROFILE=gridly  # submit to Apple, wait for result
